...
public class PopLayout extends LinearLayout {
  ...
  public PopLayout(Context context, @Nullable AttributeSet attrs) {
    super(context, attrs);
    init(context, attrs);
  }

  private void init(Context context, AttributeSet attrs) {
    TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.LibUIComponentsPopLayout);
    // 背景颜色
    @SuppressLint("ResourceAsColor")
    int backGroundColor =
        ta.getColor(R.styleable.LibUIComponentsPopLayout_lib_ui_components_background_color, R.color.lib_ui_components_tips_color);
    // 阴影颜色
    int shadowColor =
        ta.getColor(R.styleable.LibUIComponentsPopLayout_lib_ui_components_background_color, Color.parseColor("#999999"));
    int defShadowSize =
        (int)
            TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_PX, 4, getResources().getDisplayMetrics());
    // 阴影尺寸
    int shadowSize =
            ta.getDimensionPixelSize(R.styleable.LibUIComponentsPopLayout_lib_ui_components_shadow_size, defShadowSize);
    mRadius = ta.getDimensionPixelSize(R.styleable.LibUIComponentsPopLayout_lib_ui_components_radius, 0);
    // 三角形方向
    mDirection = ta.getInt(R.styleable.LibUIComponentsPopLayout_lib_ui_components_direction, BOTTOM);
    mOffset = ta.getDimensionPixelOffset(R.styleable.LibUIComponentsPopLayout_lib_ui_components_offset, 0);
    ta.recycle();
