	...
	private void addEllipsisAndAllAtEnd(){
		if (maxShowLines > 0 && maxShowLines < getLineCount()) {
			try {
				...
				SpannableString sb2 = new SpannableString("#北美好外教#   ");
				sb.setSpan(new ShowAllSpan(getContext(), onAllSpanClickListener), 0, sb2.length(), Spanned.SPAN_INCLUSIVE_INCLUSIVE);
				this.append(sb2,0,sb2.length());
			}catch (Exception e){}
		}
	}
